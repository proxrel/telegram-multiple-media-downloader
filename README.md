...
On first run, if no `.env` file is found, the script will ask you to enter your `API_ID` and `API_HASH` directly in the terminal and save them to a `.env` file automatically, so you won't be asked again on future runs.

You can find your `API_ID` and `API_HASH` at [my.telegram.org](https://my.telegram.org):

1. Log in with your Telegram account (phone number + the code sent to you).
2. Click "API development tools".
3. Fill in the form (e.g. "App title" and "Short name" — any name works).
4. Click "Create application".
5. Copy the **api_id** and **api_hash** values shown on the resulting page.

Alternatively, you can create the `.env` file yourself beforehand:
...
This project is licensed under GPLv3 License. See the [LICENSE](https://github.com/proxrel/telegram-multiple-media-downloader/blob/master/LICENSE) file for details.
