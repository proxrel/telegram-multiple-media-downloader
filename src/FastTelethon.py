...
# Source gist: https://gist.github.com/painor/7e74de80ae0c819d3e9abcf9989a8dd6
# (Based on parallel_file_transfer.py from mautrix-telegram, distributed under the MIT license with permission.)
import asyncio
import hashlib
import inspect
import logging
...
class ParallelTransferrer:
    ...
    async def _create_sender(self) -> MTProtoSender:
        ...
        if not self.auth_key:
            # Exporting + importing a cross-DC authorization must be
            # serialized per DC. ExportAuthorizationRequest issues a
            # single-use token, and self.client._init_request is a
            # single shared object on the client - two concurrent
            # ParallelTransferrer instances (e.g. two files downloading
            # at once in the same batch) racing through this block can
            # overwrite each other's `.query` before `sender.send(req)`
            # reads it, or exhaust each other's export token, which
            # surfaces as "The provided authorization is invalid"
            # (InitConnectionRequest/ImportAuthorizationRequest) on the
            # loser. The lock below ensures only one export/import
            # round-trip per DC happens at a time.
            async with parallel_transfer_locks[self.dc_id]:
